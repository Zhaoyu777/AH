...
class LogoutSuccessHandler extends DefaultLogoutSuccessHandler
{
    public function onLogoutSuccess(Request $request)
    {
        ...
        $casServer = $this->getCasServiceConfig();

        if (!empty($casServer) && !empty($casServer['enabled']) && $casServer['enabled'] == 1) {
            $response = new RedirectResponse($this->httpUtils->generateUri($request, 'sso_logout'));
    ...
    protected function getCasServiceConfig()
    {
        if (ServiceKernel::instance()->hasParameter("casServer")) {
            return ServiceKernel::instance()->getParameter("casServer");
        }
