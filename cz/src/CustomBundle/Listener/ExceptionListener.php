...
class ExceptionListener
{
    ...
    public function onKernelException(GetResponseForExceptionEvent $event)
    {
        ...
        if (strpos($path, '/weixin') === 0 && in_array($this->getServiceKernel()->getEnvironment(), array('prod'))) {
            if ($exception instanceof NotFoundException || $exception instanceof NotFoundHttpException || $exception instanceof ResourceNotFoundException) {
                $statusCode = '404';
        ...
        if (!$request->isXmlHttpRequest()) {
            ...
            if ($statusCode === Response::HTTP_FORBIDDEN && empty($user)) {
                $redirectUrl = $this->container->get('router')->generate('login', array(
                    'goto' => $path
                ));
