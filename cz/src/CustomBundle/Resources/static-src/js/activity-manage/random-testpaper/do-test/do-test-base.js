import QuestionTypeBuilder from 'app/js/testpaper/widget/question-type-builder';
...
class DoTestBase
{
  ...
  _submitTest(url,toUrl='') {
    ...
    $.post(url,{data:values[0],questionIds:values[1],usedTime:this.usedTime,attachments:attachments})
    .done((response) => {
      // if (response.result) {
      //   emitter.emit('finish', {data: ''});
      // }
      // $('#testpaper-finished-dialog').modal('hide');
      // $('.js-content-iframe').html(response);

      $('iframe', parent.document).attr('src', toUrl);
    })
    .error(function (response) {
      notify('error', response.error.message);
    });
  ...
  _getAnswers() {
    ...
    return [JSON.stringify(values), questionIds];
  }
