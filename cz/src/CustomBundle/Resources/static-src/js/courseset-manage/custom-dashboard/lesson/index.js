...
$container.map((index, ele) => {
  console.log($(ele))
  let myChart = echarts.init(document.getElementById(`lesson-dashboard-container-${index}`));
  let taskRates = $(ele).data('finishedRate');
  let finishedNum = $(ele).data('finishedNum');
  let learnNum = $(ele).data('learnNum');
  let split = 1;
  ...
  let option = {
      tooltip : {
          ...
          formatter: function(params) {
              ...
              let titles = $(ele).data('titles');
              let remarks = $(ele).data('remarks');
  ...
  function resetContainerHeight() {
    let height = calcContainerHeight($(ele));
    $(ele).height(height);
    myChart.resize();
  }

  function calcContainerHeight(ele) {
    let length = ele.data('lesson-num');
