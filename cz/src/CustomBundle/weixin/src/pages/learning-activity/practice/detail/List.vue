  ...
  <div class="wall-comment-list">
    <div class="weui-flex wall-comment-item" @click="reply(item)" v-for="item in postData">
      ...
      <div class="weui-flex__item">
        ...
        <div class="wall-comment__info"><span v-if="item.replyName">回复 <span class="text-primary">{{ item.replyName }}</span>：</span> {{ item.content }}</div>
      </div>
...
export default {
  ...
  methods: {
    ...mapActions([types.PRACTICE_DETAIL_REPLY]),
    reply(item) {
      this[types.PRACTICE_DETAIL_REPLY]({
