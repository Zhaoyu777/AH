define(function (require, exports, module) {
  ...
  exports.run = function (options) {
    ...
    $table.on('click','.js-toogle-btn', function(e) {
      const $parent = $(this).parents('tr');
      const id = $parent.data('id');
      const $currentItems = $('#course-table').find(`.js-classroom-item__${id}`);
    ...
    $('.js-teacher-name,.js-class-name').on('input', lengthLimit);
    ...
    function lengthLimit(e) {
      ...
      if(value.length > 100) {
        const newValue = value.substr(0, 100);
        $target.val(newValue);
      }
    }
  };
